...
public class Avatar : MonoBehaviour {
    ...
    private static readonly HashSet<State> nonActionableStates = new()
	{
		State.ATTACK, State.DASHATTACK, State.SPECIAL,
		State.AERIAL, State.LANDLAG, State.TUMBLE,
		State.GROUNDSTUN, State.COMBO
	};

	private static readonly HashSet<State> nonMovableStates = new()
	{
		State.ATTACK, State.DASHATTACK, State.SPECIAL,
		State.AERIAL, State.LANDLAG, State.TUMBLE,
		State.GROUNDSTUN, State.COMBO
    ...
    Vector2 externalVelocity; // from platform
    public float Weight {
		get { return weight; }
	}
	...
	void ActionInput() {
		...
		else if (input.GetButtonDown(Button.SWAP) && currentWeapon.Equals(Weapon.RANGED))
		{
			ChangeWeapon();
		}
		else if (input.GetButtonDown(Button.RELOAD) && reloads > 0)
		{
			Reload();
			reloads--;
		}
	...
	void AddFriction() {
		...
		{
			if (Mathf.Abs(rb.linearVelocity.x) - groundTraction * Time.deltaTime <= 0.0f)
			{
				tempVec = rb.linearVelocity;
				tempVec.x = 0.0f;
				rb.linearVelocity = tempVec;
			}
			else
			{
				rb.linearVelocity -= groundTraction * Mathf.Sign(rb.linearVelocity.x) * Vector2.right * Time.deltaTime;
			}
		}
	}
		...
		if (ecb.GroundedRaycast())
		{
			if (!isGrounded && rb.linearVelocity.y < 0.0f)
			{
				...
				else
				{
					...
					else if (rb.linearVelocity.y < -hardLandThresholdSpeed)
					{
						TriggerOneFrame("HardLandTrigger");
					}
					else
	...
	void FreeFall() {
		...
		{
			rb.linearVelocity += Vector2.down * fallAccel * Time.deltaTime;
			if (rb.linearVelocity.y < -terminalFallSpeed)
			{
				tempVec = rb.linearVelocity;
		...
		if (rb.linearVelocity.x > airDriftSpeed)
		{
			rb.linearVelocity += Vector2.left * airDriftAccel * Time.deltaTime;
		}
		else if (rb.linearVelocity.x < -airDriftSpeed)
		{
			rb.linearVelocity += Vector2.right * airDriftAccel * Time.deltaTime;
		}
	}
	...
	void EndFreezeFrame() {
		...
		{
			rb.linearVelocity = knockbackToApply / 8.0f;
			techTimer = 0.0f;
	...
	public void TakeHit(int damage, float freezeTime, float stunTime, Vector2 knockback) {
		...
		print("debug magnitude knockback " + knockback.magnitude);
	...
	void ApplyGroundMotion() {
		...
		switch (currentState)
		{
			...
			case (State.DASHATTACK):
				rb.linearVelocity = dashSpeed * transform.forward;
				break;
			case (State.RUN):
				rb.linearVelocity = runSpeed * transform.forward;
				break;
			case (State.WALKFWD):
				rb.linearVelocity = walkSpeed * transform.forward;
				break;
			case (State.WALKBACK):
				rb.linearVelocity = -walkSpeed * transform.forward;
				break;
	...
	void ApplyAirMotion() {
		...
		{
			if (rb.linearVelocity.y < 0.0f && rb.linearVelocity.y > -fastFallSpeed)
			{
				tempVec = rb.linearVelocity;
	...
	// esse codigo nao eh chamado diretamente, ele eh chamado por um evento ao final da animacao de JumpSquat
	void AddJumpForce() {
		if (input.GetButtonHeld(Button.JUMP))
		{
			rb.linearVelocity += Vector2.up * fullHopSpeed;
		}
		else
		{
			rb.linearVelocity += Vector2.up * shortHopSpeed;
		}
		isGrounded = false;
	}

    // chamado por um evento de animacao
	...
	void ChangeState(State newState) {
		...
		{
			// se mudando de airborn para grounded
			if (!isGrounded)
			{
				hasDoubleJump = true;
			}
		...
		if (nonActionableStates.Contains(newState))
		{
			isActionable = false;
			isMovable = false;
		}
    ...
    void StartSpecial() {
		...
		rb.linearVelocity = specialVector * specialSpeed;
		meter -= meterCost;
	}
	...
	void ExitSpecial() {
		...
		{
			rb.linearVelocity = specialVector * (airDriftSpeed / 2.0f);
			isGrounded = false;
	...
	void ShotgunBlast() {
		...
		for (int i = 0; i < tracersPerShot; i++)
		{
			...
			tracers[ammo, i].GetComponent<Rigidbody>().linearVelocity = (tracers[ammo, i].transform.forward * shotForce * drift) + (new Vector3(rb.linearVelocity.x, rb.linearVelocity.y) * SHOT_MOMENTUM_TRANSFER_RATIO);
		}
		if (ammo <= 0 && reloads > 0)
		{
			Reload();
